...
bool ManifestService::load(const std::wstring& source, LauncherManifest& m, std::wstring& error) const {
    ...
    const auto& launcher=root.get("launcher"); next.minimumVersion=Utf8ToWide(launcher.get("minimumVersion").string()); next.updateUrl=Utf8ToWide(launcher.get("updateUrl").string("https://discord.gg/WbZarvYWgX")); next.updateMessage=Utf8ToWide(launcher.get("updateMessage").string("This loader version is outdated. Install the latest build.")); next.title=Utf8ToWide(launcher.get("branding").get("title").string("Neverlose Loader"));
    ...
    for(const auto& j:root.get("versions").array()){VersionEntry x;x.id=Utf8ToWide(j.get("id").string());x.name=Utf8ToWide(j.get("name").string());x.minecraftVersion=Utf8ToWide(j.get("minecraftVersion").string());x.loader=Utf8ToWide(j.get("loader").string());x.inheritsPreset=Utf8ToWide(j.get("inheritsPreset").string());x.mainClass=Utf8ToWide(j.get("mainClass").string());x.available=j.get("available").boolean(true);x.badge=Utf8ToWide(j.get("badge").string());for(const auto& v:j.get("arguments").array())x.arguments.push_back(Utf8ToWide(v.string()));x.artifacts=artifacts(j.get("artifacts"));if(!x.id.empty())next.versions.push_back(std::move(x));}
    const bool hasUpcoming=std::any_of(next.versions.begin(),next.versions.end(),[](const VersionEntry& x){return x.id==L"1.21.4";});
    if(!hasUpcoming){VersionEntry upcoming;upcoming.id=L"1.21.4";upcoming.name=L"Minecraft 1.21.4";upcoming.minecraftVersion=L"1.21.4";upcoming.loader=L"fabric";upcoming.inheritsPreset=L"performance";upcoming.available=false;upcoming.badge=L"Soon";next.versions.push_back(std::move(upcoming));}
