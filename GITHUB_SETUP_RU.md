...
## Как выпускать обновление лоадера

1. Измените `kLauncherVersion` в `src/LauncherCore.cpp` и версию проекта в `CMakeLists.txt`. Значения должны совпадать.
2. Соберите `Release` через `build-release.bat` или GitHub Actions. Итоговый EXE автоматически дополняется до 1 400 000 байт.
3. В GitHub откройте **Releases → Draft a new release**, создайте тег вида `loader-1.1.0` и приложите `NeverloseLoader.exe`.
4. Сначала опубликуйте Release и проверьте скачивание на чистом компьютере.
5. После проверки измените `launcher.minimumVersion` в обоих JSON-файлах папки `manifest` на номер нового релиза.
6. Проверьте `launcher.updateUrl`: `https://discord.gg/WbZarvYWgX`.
7. Закоммитьте изменения манифеста в ветку `main`. После обновления raw-файла старые поддерживающие проверку версии сборки заблокируют запуск клиента и покажут кнопку Discord.

Не повышайте `minimumVersion` до публикации рабочего EXE: иначе пользователи временно останутся без доступной сборки. Уже выпущенные версии, в которых проверки `minimumVersion` ещё не было, нельзя заблокировать новым кодом задним числом; для них можно временно включить общий режим технических работ через `maintenance.enabled`.
